...
public class Clark extends Player
{
	protected void blink(boolean on)
	{
		...
		{
			if (bulb)
				renderer.setSheet(Art.bulbClark);
			else
			{
				renderer.setSheet(Art.clark);
				//renderer.setSheet(Art.newMario);	//TEST!
			}
		} 
		else 
		{
			renderer.setSheet(Art.smallClark);
		}
	...
	public Clark(int playerIndex, int marioMode)
	{	
		super(playerIndex, marioMode);
		kind = KIND_CLARK;
	}
}

...
public class Peter extends Player
{		
	protected void blink(boolean on)
	{
		...
		{
			if (bulb)
				renderer.setSheet(Art.bulbPeter);
			else
				renderer.setSheet(Art.peter);
		} 
		else 
		{
			renderer.setSheet(Art.smallPeter);
		}
	...
	protected void calcPic() 
	{
		...
		if(isOnGround())
		{
			if(Math.abs(xa+ya)<1)
				...
				animation = new SpriteAnimation(renderer, SpriteAnimationType.Walk, true, 3);
			else
	...
	public Peter(int playerIndex, int peterMode)
	{	
		super(playerIndex, peterMode);
		kind = KIND_PETER;	
		height=24;
		canDestroyFromAbove = true;
	}
}
